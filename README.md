...
Find the corresponding sample images [here](docs/sample_data), to be used for running the sample invocations as of `calibrateCam` (i.e., although the images were acquired using a Zed 2 but were split using `splitZed` before being uploaded, with only the images of the left camera uploaded).

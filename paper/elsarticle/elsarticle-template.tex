...
\begin{figure}
    ...
    \caption{Obtaining the virtual projector. (a) The virtual camera is obtained by rotating the camera about its center of projection such that its optical axis be made parallel with the normal vector of the scene plane. (b) The virtual projector is obtained by (i) rotating the projector about the point of intersection of its optical axis with the scene plane such that its optical axis be made parallel with the scene plane's normal, (ii) rotating the $X$- and $Y$-axes to render them parallel with those of the virtual camera, and (iii) optionally translating to achieve the desired projected image dimensions. The virtual projector is thus made fronto-parallel with the ground plane and axially aligned with the virtual camera.}

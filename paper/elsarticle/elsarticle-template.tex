...
\begin{figure}
    \centering
    \subfloat[\centering Circles pattern image, in image plane of projector.]{{\includegraphics[height=3.25cm]{images/circles.png} }}
    \qquad
    \subfloat[\centering Projector calibration image (one for each target location), in image plane of camera.]{{\includegraphics[height=3.25cm]{images/detections.png}}}
...
We evaluate our approach by augmenting 15 locations across the floorspace at the Pilotfabrik using our approach and two variants of a baseline approach. At each of the 15 locations, we aim to place an image aligned with the principal axes of the floorspace and such that the image appear in each instance to be free of projective distortions and to have the same dimensions, with the horizontal axis of the image as projected to the floor measuring 30 cm. We managed to carry out all steps using our approach within a time budget of 15 min. The first variant of the baseline approach is to carry out a manual keystone correction using the software bundled with the steerable mirror system mounted alongside our projector within the same 15 min time budget; we then compare the quality of the augmentations produced with this baseline with those produced using out approach. The second variant of the baseline approach is to record how much time is necessary to produce augmentations within a tolerance of what is desired.

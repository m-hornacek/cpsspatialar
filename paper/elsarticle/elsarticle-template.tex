...
\label{re}
\end{equation}
where $\mathbf{X}_{i,j}' = \mathtt{R}_j\mathbf{X}_{i,j} + \mathbf{t}_j$ expresses $\mathbf{X}_{i,j}$ in the camera coordinate frame corresponding to the $j^{\text{th}}$ calibration image, $\mathtt{K}\mathbf{X}_{i,j}' \in \mathbb{R}^2$ gives the projection of $\mathbf{X}_{i,j}$ to that image, and $d$ is a distance function that corrects for lens distortion using the recovered lens distortion model coefficients and computes a distance with respect to the two undistorted pixel positions. The inverse rigid body transformation~$(\mathtt{R}_j^{-1}, -\mathtt{R}_j^{-1}\mathbf{t}_j^{})$ gives the pose of the $j^{\text{th}}$ camera relative to the static 3D points of the calibration pattern.
